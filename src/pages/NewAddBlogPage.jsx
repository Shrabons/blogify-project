...
const NewAddBlogPage = () => {
  ...
  const handleBlogCreate = async (createData) => {
    dispatch({ type: actions.blog.DATA_FETCHING });
    try {
      const formData = new FormData();
      formData.append("thumbnail", imgSection);
      ...
      if (response.status === 201) {
        dispatch({ type: actions.blog.BLOG_DATA_CREATE, data: response.data });
        navigate("/");
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <section>
      <div className="container">
        <form onSubmit={handleSubmit(handleBlogCreate)} className="createBlog">
          <div
